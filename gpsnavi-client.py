...
#key入力時
        ...
        elif ( key == 4 or touch_key == "S"):#隣接
            wide = WIDE
            print("wide = %d" %wide)
            time.sleep(1)
        elif (key == 5 or touch_key == "W"  ):#1本飛ばし
            wide = WIDE * 2
            print("wide = %d" %wide)
            time.sleep(1)

        elif ( key == 3 or touch_key == "C"): #0補正
                c +=  nav
...
#            time.sleep(2)
        ...
        elif ( key == 8 or touch_key == "R"):#rainfall
            os.system('wmctrl -a "sudo"' )
            for i in range(6):
                print(" ")
            try:
        ...
        elif ( key == 11 or touch_key == "M"):#面積リセット
            menseki = 0
            print("作業面積リセット")
            time.sleep(1)

        elif ( key == 10 or touch_key == "H"):#half
            c += wide/2
            print("Half Wide Offset")
            time.sleep(1)


        elif ( key == 12 or touch_key == "P"):#shp属性を取る
            shpdata=getshp()
            os.system('wmctrl -a "sudo"' )
            try:
                if shpdata !=0:
